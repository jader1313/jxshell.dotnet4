...
namespace jxshell.net4
{
	...
	public class typeDescriptor
	{
		...
		public static void addUsingsStatements(StringBuilder sb)
		{
			...
			sb.AppendLine("using jxshell.net4;");
		}
			...
			if (!this.compiled)
			{
				...
				try
				{
					...
					if(f.Exists){
						...
						Type _type = Assembly.LoadFile(file).GetType(string.Concat("jxshell.net4.", str1));
						...
						Type _type = _csharplanguage.getCompiledAssembly().GetType(string.Concat("jxshell.net4.", str));
						ConstructorInfo _constructor = _type.GetConstructor(new Type[] { typeof(Type), typeof(typeDescriptor) });
		...
		{
			sb.AppendLine("namespace jxshell.net4{");
			...
			sb.AppendLine("static jxshell.net4.invoker __invoker= new invoker();");
			sb.AppendLine("/* FIELDS */");
