...
namespace jxshell.net4
{
	[ComVisible(true)]
	[Guid("9173A427-2F3B-405D-9B0F-23C7B7048114")]
	[ProgId("jxshell.net4.Manager")]
	public class Manager : IDisposable
	{
		...
		public jxshell.net4.wrapper getObjectAsType(object o, object xt)
		{
			Type type;
			type = (!(xt is jxshell.net4.wrapper) ? this.getTypeOrGenericType(xt.ToString()) : (Type)((jxshell.net4.wrapper)xt).wrappedObject);
			object o2 = Convert.ChangeType(o, type);
			return jxshell.net4.wrapper.createWrapper(o2, typeDescriptor.loadFromType(type));
		}
		...
		public void loadManyTypes(string types)
		{
			...
			StringBuilder stringBuilder = new StringBuilder();
			jxshell.net4.typeDescriptor.addUsingsStatements(stringBuilder);
			...
			for (int i = 0; i < (int)strArrays.Length; i++)
			{
				...
				jxshell.net4.typeDescriptor typeDescriptor = new jxshell.net4.typeDescriptor(typeOrGenericType, typeName, false);
			...
			{
				csharplanguage.runScript(stringBuilder.ToString(), jxshell.net4.typeDescriptor.generateInMemory);
			}
			foreach (KeyValuePair<Type, type_1> item in dictionary)
			{
				type_1 value = item.Value;
				Type type = csharplanguage.getCompiledAssembly().GetType(string.Concat("jxshell.net4.", value.staticClass));
				ConstructorInfo constructor = type.GetConstructor(new Type[] { typeof(Type), typeof(jxshell.net4.typeDescriptor) });
		...
		public void loadManyTypes(Type[] types)
		{
			...
			for (int i = 0; i < (int)typeArray.Length; i++)
			{
				...
				{
					jxshell.net4.typeDescriptor typeDescriptor = new jxshell.net4.typeDescriptor(type, jxshell.net4.typeDescriptor.getNameForType(type), false);
					if (!typeDescriptor.isCompiled())
					{
						string staticClass = "";
						string instanceClass = "";
			...
			foreach (KeyValuePair<Type, type_1> item in dictionary)
			{
				...
				ConstructorInfo constructor = type2.GetConstructor(new Type[] { typeof(Type), typeof(jxshell.net4.typeDescriptor) });
				value.td.setCompiledWrapper((wrapperStatic)constructor.Invoke(new object[] { value.t, value.td }));
			}
		}
